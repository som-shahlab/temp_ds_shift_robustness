...
parser.add_argument(
    "--hparams_fpath",
    type=str,
    default='/local-scratch/nigam/projects/lguo/temp_ds_shift_robustness/dg/experiments/baseline/hyperparams',
...
parser.add_argument(
    ...
    type = str2bool,
    default = "false",
    help = "whether to fix the number of samples for dataloader generation",
)
...
parser.add_argument(
    ...
    type = int,
    default = 8000,
    help = "number of samples for fixed-samples dataloader generation. 8000 is a safe number to use",
)
...
def get_torch_data_loaders(
    ...
    if fixed_samples:
        df1=input_row_id_map.query(f"{task}_fold_id!='test'").sample(n=n_samples,random_state=args.seed)
        df2=input_row_id_map.query(f"{task}_fold_id=='test'")
        input_row_id_map=pd.concat((df1,df2))
