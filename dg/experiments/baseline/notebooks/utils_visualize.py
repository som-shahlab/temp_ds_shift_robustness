...
metric_labels={'auc':'AUROC','auprc':'AUPRC','ace_abs_logistic_logit':'ACE'}
...
def plot_heatmap(
    ...
    ## plot
    fig, axes = plt.subplots(nrows = 1, ncols=len(tasks),figsize=figsize)
    for c,task in enumerate(tasks):
            df = df_results.query("task==@task and metric==@metric")
            ...
            pos = axes[c].imshow(
                df.values,
                cmap=f"{cmap}" if metric in ['auc','auprc'] else f"{cmap}_r",
                vmin=vmin,
                vmax=vmax,
            )
            ...
            if c==len(tasks)-1 and y_axis!='auto':
                fig.colorbar(
                    pos,ax=axes[c],
                    label=metric_labels[metric] if dv=='comparator' else f"Relative\n{metric_labels[metric]}",
                    fraction=0.046,
                    pad=0.04
                )
            else:
            ...
            axes[c].set_title(tasks[task])
            axes[c].set_xticks(np.arange(0,len(train_year_labels),1))
            axes[c].set_xticklabels(train_year_labels)
            axes[c].set_xlabel('Train Year Groups')
            axes[c].set_ylabel(f"Evaluation Year [OOD]")
            axes[c].set_yticks(np.arange(0,len(test_group),1))
            axes[c].set_yticklabels(test_group)
