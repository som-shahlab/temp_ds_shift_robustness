...
from .prediction_model import CLMBR, BinaryLinearCLMBRClassifier
...
def read_df(dirname: str) -> pd.DataFrame:
    if dirname.split('.')[-1]=='csv':
        return pd.read_csv(dirname)
    elif dirname.split('.')[-1]=='parquet':
        return pd.read_parquet(dirname)
...
def train_model() -> None:
    ...
    parser.add_argument(
        ...
        default=6,
        type=int,
        help='number of layers to use if encoder_type is "transformer" (default 6), '
        'not used if encoder_type is "gru" or "lstm"',
    )
    parser.add_argument(
        "--transformer_heads",
        default=8,
        type=int,
        help='number of heads to use if encoder_type is "transformer" (default 8), '
        'not used if encoder_type is "gru" or "lstm"',
    )
    parser.add_argument(
        "--dropout",
...
def train_ete_model() -> None:
    parser = argparse.ArgumentParser(
        description="End-to-end model Experiments"
    )
    # paths
    parser.add_argument(
        "model_dir",
    ...
    parser.add_argument(
        "--train_labels_path",
        type=str,
        help="Path to df that contains train person_ids (after ehr_ml convert_data), day_ids, and labels",
    )
    parser.add_argument(
        "--val_labels_path",
        type=str,
        help="Path to df that contains val person_ids (after ehr_ml convert_data), day_ids, and labels",
    )
    # model specification
    parser.add_argument(
        "--size",
    ...
    # read data
    train = read_df(args.train_labels_path)
    val = read_df(args.val_labels_path)
    ...
    dataset = PatientTimelineDataset(
        ...
        os.path.join(info["extract_dir"], "ontology.db"),
        os.path.join(args.info_dir, "info.json"),
        (train['labels'].to_numpy(), train['person_ids'].to_numpy(), train['day_ids'].to_numpy()),
        (val['labels'].to_numpy(), val['person_ids'].to_numpy(), val['day_ids'].to_numpy()),
    )
    ...
    model = model.load_weights(model_dir)
